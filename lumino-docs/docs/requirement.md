...
## Usabilidad
Desde el punto de vista de la usabilidad, Lumino ofrece una interfaz clara y coherente, donde los usuarios acceden siempre a las funcionalidades relevantes para su perfil. Tras el inicio de sesión, cada usuario es redirigido a un panel principal adaptado a su rol, evitando menús innecesarios o acciones que no le corresponden. Esta separación reduce la complejidad de uso y facilita que alumnos y profesores puedan interactuar con la plataforma de forma intuitiva, sin requerir conocimientos técnicos previos.

## Seguridad
En cuanto a la seguridad, Lumino se apoya en los mecanismos nativos de Django para controlar el acceso a la aplicación. Las vistas sensibles están protegidas mediante el decorador `#!python @login_required` , lo que garantiza que únicamente usuarios autenticados puedan acceder a ellas. Además, el sistema implementa restricciones de acceso por roles, asegurando que cada acción solo pueda ser ejecutada por el tipo de usuario autorizado. Por ejemplo, los alumnos no pueden modificar temarios ni calificaciones, y los profesores no pueden realizar acciones administrativas fuera de su ámbito.
...
- [x] [Restricciones técnicas](#restricciones-tecnicas){ data-preview }
- [x] [Restricciones econćomicas](#restricciones-economicas){ data-preview }
- [x] [Factores temporales](#factores-temporales){ data-preview }
- [x] [Aspectos legales](#aspectos-legales){ data-preview }
- [x] [Restricciones organizativas](#restricciones-organizativas){ data-preview }
